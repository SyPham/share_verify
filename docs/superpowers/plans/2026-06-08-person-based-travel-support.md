...
**Goal:** Chuyển luồng xác minh ShareVerify sang Quét mã cổ đông (MCD) là bước đầu tiên, xác minh giấy tờ chỉ làm bằng chứng, và chặn trùng phụ cấp theo Person (không theo MCD hay số giấy tờ).
...
```
Quét mã cổ đông (MCD) → Hiển thị cổ đông → Xác minh giấy tờ (4 cách)
...
|---------|-----------|----------|
| 1. Quét mã cổ đông | `VerificationBarcodeSection` (mới) | Nút quét barcode → parse MCD → `GET /api/shareholders/{mcd}` |
...
```dart
// Nút chính: "Quét mã cổ đông" — icon barcode_scanner
...
Tap "Quét mã cổ đông" → hiển thị cổ đông MCD001
